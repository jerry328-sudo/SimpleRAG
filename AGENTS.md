...
- 本项目后续的需求讨论、架构设计、代码实现、代码评审与发布前检查，除遵循本 `AGENTS.md` 外，还必须遵循：
  - `doc/11-SimpleRAG-RAG-详细设计与实施计划.md`
  - `doc/12-SimpleRAG-项目规范与边界.md`
- `doc/11-SimpleRAG-RAG-详细设计与实施计划.md` 是本项目的主设计文档，用于约束产品目标、模块划分、数据模型、索引/检索/聊天流程与实施顺序。
- `doc/12-SimpleRAG-项目规范与边界.md` 是本项目的边界与评审文档，用于约束实现范围、数据边界、索引边界、聊天边界、Provider 边界、测试边界与需求变更边界。
- 如果实现方案、需求变更或代码行为与 `doc/11` 或 `doc/12` 冲突，应先更新对应文档并确认新的边界，再修改实现；不允许先偏离文档落地，再事后补文档。
